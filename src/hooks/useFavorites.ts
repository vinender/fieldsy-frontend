  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5001/api'}/favorites/toggle/${fieldId}`,
        {
          method: 'POST',
          headers: {
...
export function useFavoriteStatus(fieldId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5001/api'}/favorites/check/${fieldId}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
...
export function useSavedFields(page: number = 1, limit: number = 10) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5001/api'}/favorites/my-saved-fields?page=${page}&limit=${limit}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
...
export function useRemoveFavorite() {
  ...
  return useMutation({
    mutationFn: async (fieldId: string) => {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5001/api'}/favorites/${fieldId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,

...
export default function FieldDetailsLegacy({ field, isSubmitted = false, isPreview = false, headerContent, showReviews = true, showOwnerInfo = true, showClaimField = true }: FieldDetailsLegacyProps) {
  ...
  const getFieldSizeDisplay = () => {
    ...
    if (isPresetOption) {
      // Use the dropdown preset value with label formatting
      return getFieldPropertyLabel('fieldSize', sizeValue);
              ...
              <div className="h-full flex flex-col space-y-4 lg:sticky lg:top-24">
                <div className="grid grid-cols-2 gap-3 lg:gap-4">
                  {fieldImages?.slice(0, 4).map((img: string, index: number) => {
                    ...
                    const hasMoreImages = hasRealImages && fieldImages.length > 4;
                    ...
                    return (
                      ...
                      >
                        ...
                        <Image
                          ...
                          className={`object-cover ${hasRealImages ? 'group-hover:scale-105' : ''} transition-transform duration-300 ${showMoreButton ? 'brightness-75' : ''} ${!isImageLoaded ? 'opacity-0' : 'opacity-100'}`}
                        />

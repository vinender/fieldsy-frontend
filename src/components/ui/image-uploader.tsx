...
import { X, Upload, AlertCircle, Check, Image as ImageIcon } from 'lucide-react'
...
const DEFAULT_ACCEPTED_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp', 'image/avif']
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        ...
        'image/avif',
        0.80 // Quality: 80% (AVIF is more efficient, similar visual quality at lower percentage)
      )
    }
...
export function ImageUploader({
  ...
  const handleFiles = async (files: File[]) => {
    ...
    for (const file of files.slice(0, availableSlots)) {
      ...
      try {
        // Convert to AVIF format (with WebP fallback)
            ...
            {placeholder || (
              <>
                ...
                <p className="text-sm text-gray-600">
                  Supports: JPG, PNG, GIF (Max {maxSize}MB) - Auto-converts to AVIF
                </p>
                {multiple && (

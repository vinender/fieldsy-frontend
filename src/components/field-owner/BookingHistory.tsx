...
export default function BookingHistory() {
  ...
  useEffect(() => {
    if (highlightedBookingId && !isLoading) {
      ...
      if (targetBooking) {
        setSelectedBooking(targetBooking as any);
        setIsModalOpen(true);
                    ...
                    <div key={displayBooking.id} className="bg-white shadow-md rounded-xl sm:rounded-2xl p-4 sm:p-5 hover:shadow-lg transition-shadow">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 border-b border-b-gray-200 pb-3 sm:pb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {activeTab === 'today' && (
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase flex-shrink-0 ${displayBooking.status === 'confirmed' ? 'bg-blue-100 text-blue-700' :
                                displayBooking.status === 'completed' ? 'bg-green/10 text-green' :
                                  displayBooking.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                                    displayBooking.status === 'cancelled' ? 'bg-red-100 text-red-600' :
                                      displayBooking.status === 'refunded' ? 'bg-orange-100 text-orange-600' :
                      ...
                      <div className="flex gap-2">
                        {activeTab === 'completed' || activeTab === 'cancelled' ? (
                          <>
                            <div className={`flex-1 py-2 px-3 border rounded-full text-xs font-medium font-sans flex items-center justify-center gap-1.5 ${displayBooking.status === 'completed' ? 'bg-green/10 text-green' :
                  ...
                  {[...Array(Math.min(5, cancelledPagination.totalPages))].map((_, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 sm:w-10 h-8 sm:h-10 rounded-full text-xs sm:text-sm md:text-base font-medium transition-all font-sans ${cancelledPage === pageNum
                          ? 'bg-green text-white'
                  ...
                  {[...Array(Math.min(5, totalPages))].map((_, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 sm:w-10 h-8 sm:h-10 rounded-full text-xs sm:text-sm md:text-base font-medium transition-all font-sans ${currentPage === pageNum
                          ? 'bg-green text-white'
                          : 'bg-white text-dark-green hover:bg-cream'
                          }`}
                      >
